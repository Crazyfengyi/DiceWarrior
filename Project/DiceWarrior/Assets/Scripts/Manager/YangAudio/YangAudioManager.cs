...
namespace YangTools.Scripts.Core.YangAudio
{
    ...
    /// 声音管理器
    /// </summary>
    public class YangAudioManager : MonoSingleton<YangAudioManager>
    {
        #region 属性
        ...
        private static async UniTask<AudioClip> GetAudioClip(string audioName)
        {
            ...
            {
                if (LoadingAudioNames.Contains(audioName))
                {
                    //Debug.LogError("正在加载声音资源：" + audioName);
                    //防止重复加载
                ...
                if (target != null)
                {
                    allAudioClipsDic.Add(audioName, target);
                    return target;
                }
            }

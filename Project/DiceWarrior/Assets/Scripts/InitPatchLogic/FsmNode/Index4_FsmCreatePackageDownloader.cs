...
namespace GameMain
{
    public class FsmCreatePackageDownloader : IStateNode
    {
        ...
        IEnumerator CreateDownloader()
        {
            ...
            ResourceDownloaderOptions tempData = new ResourceDownloaderOptions(downloadingMaxNum,failedTryAgain);
            var downloader = package.CreateResourceDownloader(tempData);
            machine.SetBlackboardValue("Downloader", downloader);

            if (downloader.TotalDownloadCount == 0)
            {
                Debug.LogWarning("YooAsset:不需要下载资源");
            ...
            else
            {
                ...
                Debug.LogWarning($"YooAsset: 需要下载资源 totalDownloadCount:{totalDownloadCount} totalDownloadBytes:{totalDownloadBytes}");
